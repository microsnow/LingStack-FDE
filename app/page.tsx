...
const tools: Tool[] = [
  ...
  { id:"cron",name:"Cron 解析",desc:"解读五段 Cron 表达式",icon:"⏱",color:"pink",category:"后端工具",tag:"新" },
...
export default function Home(){
  ...
  async function runTool(action="primary"){
    try{setError("");let result="";
      ...
      else if(active==="cron"){const p=input.trim().split(/\s+/);if(p.length!==5)throw new Error("请输入标准五段 Cron 表达式");const labels=["分钟","小时","日","月","星期"];result=p.map((x,i)=>`${labels[i]}：${x==="*"?"每一":x.startsWith("*/")?`每 ${x.slice(2)} 个单位`:x}`).join("\n")+"\n\n提示：下次执行时间受服务器时区影响。"}
  ...
  return <main className="app-shell">
    ...
    <aside className="sidebar"><nav>{categories.map((c,i)=><button onClick={()=>setCategory(c)} className={category===c?"selected":""} key={c}><span>{icons[i]}</span>{c}<em>{c==="常用收藏"?favorites.length:c==="最近使用"?recent.length:""}</em></button>)}</nav><div className="privacy"><span>◉</span><div><b>隐私优先</b><p>输入内容仅在本机浏览器中处理。</p></div></div><div className="version"><span>DevKit v0.3.0</span><span className="status-dot"/>运行正常</div></aside>
    <section className="content"><div className="welcome"><div><p className="eyebrow">DEVKIT V0.3 · LOCAL FIRST</p><h1>今天想解决什么问题？</h1><p>选择工具，输入数据，立即得到结果。</p></div><div className="pulse"><i/><span>{tools.length}</span><small>已上线工具</small></div></div>
      ...
      <section className="workspace"><div className="workspace-head"><div className={`tool-icon ${current.color}`}>{current.icon}</div><div><p>{current.category}</p><h2>{current.name}</h2></div><button className={`head-favorite ${favorites.includes(active)?"on":""}`} onClick={()=>toggleFavorite(active)}>★ {favorites.includes(active)?"已收藏":"收藏"}</button><span className="secure">● 本地安全处理</span></div>
        ...
        <div className="workspace-actions"><button className="primary" onClick={()=>runTool()}>执行转换 <kbd>Ctrl ↵</kbd></button>{active==="json"&&<button onClick={()=>runTool("minify")}>压缩 JSON</button>}{["base64","url","unicode","entities"].includes(active)&&<button onClick={()=>runTool("decode")}>解码</button>}{active==="units"&&<button onClick={()=>runTool("decode")}>rem 转 px</button>}{active==="hash"&&<><button onClick={()=>runTool("sha1")}>SHA-1</button><button onClick={()=>runTool("sha512")}>SHA-512</button></>}{active==="text"&&<><button onClick={()=>runTool("upper")}>大写</button><button onClick={()=>runTool("lower")}>小写</button><button onClick={()=>runTool("unique")}>去重复行</button><button onClick={()=>runTool("sort-asc")}>升序</button><button onClick={()=>runTool("sort-desc")}>降序</button></>}</div>
      </section>
    </section>
  </main>;
}
