...
function encodeBase64(v: string) {
...
function inferType(v: unknown): string {
  if (Array.isArray(v)) return `${v.length ? inferType(v[0]) : "unknown"}[]`;
  if (v === null) return "null";
  if (typeof v === "object")
    return `{ ${Object.entries(v as Record<string, unknown>)
      .map(([k, x]) => `${k}: ${inferType(x)}`)
      .join("; ")} }`;
  return typeof v;
}
  ...
  if (v && typeof v === "object")
    return Object.entries(v as Record<string, unknown>)
      .map(([k, x]) =>
        x && typeof x === "object"
          ? `${pad}${k}:\n${toYaml(x, depth + 1)}`
          : `${pad}${k}: ${typeof x === "string" ? JSON.stringify(x) : String(x)}`,
      )
      .join("\n");
  return `${pad}${String(v)}`;
}
function ipNumber(ip: string) {
  ...
  if (p.length !== 4 || p.some((x) => x < 0 || x > 255 || !Number.isInteger(x)))
    throw new Error("请输入有效 IPv4 地址");
  return p.reduce((n, x) => (n * 256 + x) >>> 0, 0) >>> 0;
}
function numberIp(n: number) {
  return [24, 16, 8, 0].map((s) => (n >>> s) & 255).join(".");
}
function luminance(hex: string) {
  const h = hex.replace("#", "");
  if (!/^[0-9a-f]{6}$/i.test(h)) throw new Error("请输入两个 6 位 HEX 颜色");
  const c = [0, 2, 4]
...
function rmbUppercase(value: string) {
  ...
  if (amount > 999999999999999.99)
    throw new Error("金额不能超过 999999999999999.99 元");
  ...
  do {
    ...
    let section = "",
      zero = false;
    for (let i = 0, n = part; i < 4; i++, n = Math.floor(n / 10)) {
      const d = n % 10;
      if (d) {
        section = digits[d] + units[3 - i] + section;
        zero = false;
  ...
  sections.forEach((section, i) => {
    if (section.text) {
      ...
      text += section.text + sectionUnits[sections.length - 1 - i];
    }
  });
...
async function describeCron(
  expression: string,
  mode: "linux" | "spring" | "quartz",
) {
  const { default: cronstrue } = await import("cronstrue/i18n.js");
  const fields = expression.trim().split(/\s+/),
    expected = mode === "linux" ? "5" : mode === "spring" ? "6" : "6 或 7";
  if (
    ...
    throw new Error(
      `${mode === "linux" ? "Linux" : mode === "spring" ? "Spring" : "Quartz"} 表达式需要 ${expected} 个字段`,
    );
  const names =
    mode === "linux"
      ? ["分钟", "小时", "日", "月", "星期"]
  ...
  try {
    ...
    throw new Error("表达式格式无效，请检查字段范围和特殊字符");
  }
  const title =
    mode === "linux"
      ? "Linux Crontab"
...
function parseDateValue(raw: string) {
  ...
  if (Number.isNaN(date.getTime())) throw new Error(`无法识别日期：${value}`);
  return date;
}
...
export default function Home({
  ...
  async function runTool(action = "primary") {
    try {
      setError("");
      let result = "";
      const p0Result = await runP0Tool(active, input, action);
      if (p0Result !== null) {
        setOutput(p0Result);
        return;
      }
      if (active === "json")
        ...
        result =
          action === "decode" ? decodeBase64(input) : encodeBase64(input);
      else if (active === "url")
        result =
          action === "decode"
            ? decodeURIComponent(input)
            : encodeURIComponent(input);
      else if (active === "unicode")
        result =
          action === "decode"
            ? input.replace(/\\u([0-9a-fA-F]{4})/g, (_, h) =>
        ...
        const d = Number.isFinite(n)
          ? new Date(input.trim().length <= 10 ? n * 1000 : n)
          : new Date(input);
        ...
        const values = input
          ...
          .filter(Boolean);
        if (values.length !== 2)
          throw new Error("请输入两个日期时间，每行一个");
        ...
        const values = input
          ...
          .map((x) => x.trim())
          .filter(Boolean);
        if (!values.length) throw new Error("请至少输入一个日期或时间戳");
        result = values
          .map(
            (value, index) =>
              `# ${index + 1} · ${value}\n${dateDetails(parseDateValue(value))}`,
          )
          .join("\n\n");
      } else if (active === "radix") {
        const n = input.trim().startsWith("0x")
          ? parseInt(input, 16)
          : Number(input);
        if (!Number.isInteger(n)) throw new Error("请输入有效整数");
        result = `二进制：${n.toString(2)}\n八进制：${n.toString(8)}\n十进制：${n}\n十六进制：${n.toString(16).toUpperCase()}`;
      } else if (active === "chinese")
        result = await convertChinese(input, action === "simplified");
      else if (active === "case")
        result =
          action === "lower"
        ...
        if (action === "decode") {
          ...
          if (
            !values.length ||
            values.some((x) => !/^\d+$/.test(x) || Number(x) > 0x10ffff)
          )
            throw new Error("请输入以空格或逗号分隔的十进制编码数字");
          result = values.map((x) => String.fromCodePoint(Number(x))).join("");
        ...
        result = [...new Uint8Array(digest)]
          ...
          .join("");
      } else if (active.startsWith("md5-")) {
        const { default: CryptoJS } = await import("crypto-js");
        const full = CryptoJS.MD5(input).toString();
        result = active.includes("16-") ? full.slice(8, 24) : full;
        if (active.endsWith("upper")) result = result.toUpperCase();
      } else if (active === "sha1") {
        const { default: CryptoJS } = await import("crypto-js");
        result = CryptoJS.SHA1(input).toString();
      } else if (active === "sha2-256") {
        const { default: CryptoJS } = await import("crypto-js");
        result = CryptoJS.SHA256(input).toString();
      } else if (active === "sha2-512") {
        const { default: CryptoJS } = await import("crypto-js");
        result = CryptoJS.SHA512(input).toString();
      } else if (active.startsWith("sha3-")) {
        ...
        result = Array.from(
          algorithms[active as keyof typeof algorithms](
            new TextEncoder().encode(input),
          ),
          (byte) => byte.toString(16).padStart(2, "0"),
        ).join("");
      } else if (active === "ripemd-160") {
        const { default: CryptoJS } = await import("crypto-js");
        result = CryptoJS.RIPEMD160(input).toString();
      } else if (active === "crc32-64")
        result = crc32(input).toString(16).padStart(16, "0");
      else if (
        /^(aes|des|tripledes|rabbit|rc4)-(encrypt|decrypt)$/.test(active)
      ) {
        const [name, mode] = active.split("-") as [
          CipherName,
          "encrypt" | "decrypt",
        ];
        result = await cipherRun(name, mode === "decrypt", input);
      } else if (active === "uuid")
        result = Array.from(
          { length: Math.min(Math.max(Number(input) || 1, 1), 100) },
          () => crypto.randomUUID(),
        ).join("\n");
        ...
        const definitions = {
          ...
          groups = Object.entries(passwordGroups)
            .filter(([, enabled]) => enabled)
            .map(([name]) => definitions[name as keyof typeof definitions]);
        if (!groups.length) throw new Error("请至少选择一种字符类型");
        if (passwordLength < groups.length)
          throw new Error(
            `密码长度不能少于已选择的 ${groups.length} 种字符类型`,
          );
        result = Array.from({ length: passwordCount }, () =>
          generatePassword(passwordLength, groups),
        ).join("\n");
      } else if (active === "text") {
        ...
        result = JSON.stringify(
          { header: parse(h), payload: parse(p), notice: "仅解析，未验证签名" },
          null,
          2,
        );
      } else if (active === "json2ts") {
        const v = JSON.parse(input);
        result = `interface Root ${inferType(v)}`;
      } else if (active === "query") {
        const u = new URL(
          input.includes("://")
            ? input
            : `https://local.dev/?${input.replace(/^\?/, "")}`,
        );
        result = JSON.stringify(Object.fromEntries(u.searchParams), null, 2);
        ...
        if (action === "rgb" || input.trim().startsWith("#")) {
          ...
          result = `rgb(${parseInt(full.slice(0, 2), 16)}, ${parseInt(full.slice(2, 4), 16)}, ${parseInt(full.slice(4, 6), 16)})`;
        } else {
          const m = input.match(/-?\d+(?:\.\d+)?/g);
          if (
            !m ||
            m.length !== 3 ||
            m.some((x) => Number(x) < 0 || Number(x) > 255)
          )
            throw new Error("请输入三个 0–255 之间的 RGB 数值");
          result =
            "#" +
            m
        ...
        const [a = "", b = ""] = input.split(/^--- 对比 ---$/m),
          aa = a.trim().split("\n"),
          bb = b.trim().split("\n"),
          size = Math.max(aa.length, bb.length);
        ...
        const method =
          input.match(/(?:-X|--request)\s+([A-Z]+)/i)?.[1] ??
          (input.match(/(?:-d|--data)/) ? "POST" : "GET");
        ...
        result = `fetch(${JSON.stringify(url)}, ${JSON.stringify({ method: method.toUpperCase(), ...(Object.keys(headers).length ? { headers } : {}), ...(body ? { body } : {}) }, null, 2)})\n  .then(response => response.json())\n  .then(console.log);`;
      } else if (active === "cidr") {
        ...
        result = `网络地址：${numberIp(network)}\n子网掩码：${numberIp(mask)}\n广播地址：${numberIp(broadcast)}\n可用范围：${numberIp(prefix >= 31 ? network : network + 1)} — ${numberIp(prefix >= 31 ? broadcast : broadcast - 1)}\n主机数：${hosts}`;
      } else if (active === "units") {
        const n = Number(input);
        if (!Number.isFinite(n)) throw new Error("请输入数值");
        result =
          action === "decode"
            ? `${n}rem = ${n * 16}px（基准 16px）`
            : `${n}px = ${n / 16}rem（基准 16px）`;
      } else if (active === "contrast") {
        const [a, b] = input.trim().split(/\s+/),
          l1 = luminance(a),
          l2 = luminance(b),
          ratio = (Math.max(l1, l2) + 0.05) / (Math.min(l1, l2) + 0.05);
        result = `对比度：${ratio.toFixed(2)}:1\n普通文本 AA：${ratio >= 4.5 ? "通过" : "不通过"}\n大文本 AA：${ratio >= 3 ? "通过" : "不通过"}\nAAA：${ratio >= 7 ? "通过" : "不通过"}`;
      } else if (active === "sql")
        result = input
          .replace(/\s+/g, " ")
          .replace(
            /\b(SELECT|FROM|WHERE|LEFT JOIN|RIGHT JOIN|INNER JOIN|JOIN|GROUP BY|ORDER BY|HAVING|LIMIT|VALUES|SET)\b/gi,
            "\n$1",
          )
          .replace(/\b(AND|OR)\b/gi, "\n  $1")
          .trim();
      else if (active === "yaml") result = toYaml(JSON.parse(input));
      setOutput(result);
    } catch (e) {
      setOutput("");
      setError(e instanceof Error ? e.message : "处理失败，请检查输入。");
    }
  }
