...
- 桌面端支持通过文件菜单、系统文件关联和拖放打开文本与数据文件
- 桌面端凭据使用 Windows DPAPI、macOS Keychain 或 Linux 系统密钥环加密保存
- 桌面端可从 GitHub Releases 检查、下载和安装更新
- 提供 Windows、macOS 与 Linux 桌面打包配置和平台适配层
...
- 主题、收藏和最近使用仅保存在当前浏览器
- 清理浏览器网站数据后，本地偏好会被重置
- 当前 78 个工具全部归类为本地工具；未来联网与 AI 工具必须在界面中明确标识数据去向
